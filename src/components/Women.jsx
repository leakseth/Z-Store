...
function MenProductCard({ item, index }) {
  return (
    <motion.div
      ...
      viewport={{once: true}}
    >
        <Link to={`/product/${item.id}`} onClick={window.scroll(0,0)}
...
function Women() {
  const menProducts = data_product.filter(item => item.category === "women");
      ...
      <div className="w-[90%] md:w-[80%] mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {menProducts.map((item, index) => (
            <MenProductCard key={item.id} item={item} index={index} />
          ))}
        </div>
      </div>
    </section>
